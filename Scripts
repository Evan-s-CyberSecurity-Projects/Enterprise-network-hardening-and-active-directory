...
## 2. Safe Domain Controller Rename Script
For Domain Controllers, use `netdom` to prevent Active Directory replication breaks.

```powershell
# Run on the DC as Domain Administrator
\$OldFQDN = "DC-OLD.enterprise.local"
\$NewFQDN = "DC-NEW.enterprise.local"

# Add the new alternate name
netdom computername \(OldFQDN /add:\)NewFQDN

# Make the new name primary (Requires Reboot after execution)
